...
public class BaseRequest<T> extends BaseRequestParser {
   ...
   public void callAPIPostImage(final int requestCode, RequestBody image, String remainingURL, RequestBody input1, RequestBody input2, RequestBody input3) {
        RequestCode = requestCode;
        showLoader();
        Log.d("BaseReq", "Input URL : " + remainingURL);
        String sess = SessionParam.getSessionKey(mContext);
        callAPI = apiInterface.uploadImage(remainingURL, image,input1,input2,input3, sess);
        callAPI.enqueue(responseCallback);
    }
